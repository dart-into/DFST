...
ResultSave_path='record_livec.txt'
...
class Net(nn.Module):
    ...
    def feature_genetic(self, x):
        ...
        channel_num = torch.randperm(num_channels)[:num_splits]
        new_random_indices = torch.randperm(512)
        channel_indices1 = channel_num[new_random_indices]
        new_random_indices2 = torch.randperm(512)
        channel_indices = channel_indices1[new_random_indices2]
...
class range_module(nn.Module):
    def __init__(self, pretrained=True):
        super().__init__()
        # Load pre-trained DenseNet
        self.densenet = models.densenet121(pretrained=True)
        
        # Extract different dense blocks' outputs
        self.denseblock1_out = nn.Sequential(*list(self.densenet.features.children())[:4])
        self.denseblock2_out = nn.Sequential(*list(self.densenet.features.children())[4:6])
        self.denseblock3_out = nn.Sequential(*list(self.densenet.features.children())[6:8])
        self.denseblock4_out = nn.Sequential(*list(self.densenet.features.children())[8:10])

    def forward(self, x):
        # Forward pass through the entire DenseNet
        denseblock1_out = self.denseblock1_out(x)
        denseblock2_out = self.denseblock2_out(denseblock1_out)
        denseblock3_out = self.denseblock3_out(denseblock2_out)
        denseblock4_out = self.denseblock4_out(denseblock3_out)
        
        return denseblock1_out, denseblock2_out, denseblock3_out, denseblock4_out
...
def finetune_model():
    ...
    data_dir = os.path.join('/home/user/use_trans/CSIQ/')
    images = pd.read_csv(os.path.join(data_dir, 'image_labeled_by_score.csv'), sep=',')
    images_fold = "/home/user/use_trans/CSIQ/"
    ...
    for i in range(10):
        ...
        for epoch in range(epochs):
            ...
            if sp > best_srocc:
                ...
                if best_srocc > 0.94:
                    torch.save(best_model.cuda(),'model_IQA/dct_livec_feature.pt')
...
def load_data(mod = 'train'):
    ...
    transformed_dataset_train3 = ImageRatingsDataset(csv_file=train_path,
                                                    root_dir='/home/user/data/LCSIQ/non_salient_image/',
                                                    transform=transforms.Compose([Rescale(output_size=(300, 300)),
                                                                                  RandomHorizontalFlip(0.5),
                                                                                  RandomCrop(output_size=output_size),
                                                                                  Normalize(),
