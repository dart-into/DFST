...
class Net(nn.Module):
    def __init__(self, net1, net2, net3, net4, vit, linear):
    ...
    def forward(self, x1, x2, x3):
        ...
        #x2 = self.CONV1(x2)
        #x2 = self.Net2(x2)
        x3 = self.Net3(x3)  #n_sa
        #x4 = self.CONV2(x4)
        #x4 = self.Net1(x4)
        x1 = self.Net5(x1)    #512,16,16 sample
        
        #print(x1.shape)
        x1 = self.feature_genetic(x1)
        #x2 = self.feature_genetic(x2)
        x2 = self.feature_genetic(x2)
        x3 = self.feature_genetic(x3)  #128,14,14
        
        features2 = torch.cat((x2, x3, x1), dim=1) #384,14,14
...
def computeSpearman(dataloader_valid1, dataloader_valid2, dataloader_valid3, model):
    ...
    with torch.no_grad():
        ...
        for data1, data2, data3 in tqdm(zip(dataloader_valid1, dataloader_valid2, dataloader_valid3), total=total_iterations, desc='Processing'):
            inputs1 = data1['image']
...
def train_model():
    ...
    model = Net(net1 = net_1, net2 = net_2, net3 = net_3, net4 = net_4, vit = VIT, linear = l_net)
    '''
    ...
    torch.save(model.cuda().state_dict(), 
               'model_IQA/TID2013_KADID10K_4_1.pt')
...
def load_data(mod = 'train', dataset = 'tid2013', worker_idx = 0):
    ...
    if dataset == 'tid2013':
        data_dir = os.path.join('/home/user/DFST-master/tid2013')
        ...
        data_dir = os.path.join('/home/user/DFST-master/kadid10k')
        worker_orignal = pd.read_csv(os.path.join(data_dir, 'image_labeled_by_per_noise.csv'), sep=',')
